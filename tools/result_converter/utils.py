...
def get_lidar_3d_8points(label_3d_dimensions, lidar_3d_location, rotation_z):
    ...
    lidar_3d_8points = (
        lidar_rotation * corners_3d_lidar + np.matrix(lidar_3d_location).T
    )
...
def get_label_lidar_rotation(lidar_3d_8_points):
    ...
    dx, dy = x0 - x3, y0 - y3
    rotation_z = math.atan2(
        dy, dx
    )  # Lidar坐标系xyz下的偏航角yaw绕z轴与x轴夹角，方向符合右手规则，所以用(dy,dx)
    return rotation_z
...
def get_camera_3d_8points(label_3d_dimensions, camera_3d_location, rotation_y):
    ...
    camera_3d_8points = (
        camera_rotation * corners_3d_camera + np.matrix(camera_3d_location).T
    )
...
def get_camera_3d_alpha_rotation(camera_3d_8_points, camera_3d_location):
    ...
    rotation_y = -math.atan2(
        dz, dx
    )  # 相机坐标系xyz下的偏航角yaw绕y轴与x轴夹角，方向符合右手规则，所以用(-dz,dx)
    # alpha = rotation_y - math.atan2(center_in_cam[0], center_in_cam[2])
    alpha = (
        rotation_y
        - (-math.atan2(-camera_3d_location[2], -camera_3d_location[0]))
        + math.pi / 2
    )  # yzw
    # add transfer
    if alpha > math.pi:
